...
double	cone_inter(t_env *e, t_ray *ray)
{
	...
	//ocos[x] = cos(ft_deg2rad(((t_obj*)e->objs->content)->rad));
	//osin[x] = sin(ft_deg2rad(((t_obj*)e->objs->content)->rad));
	temp = vector_substraction(e->cam.pos, ((t_obj*)e->objs->content)->pos);
	/*r = (double)((t_obj*)e->objs->content)->rad * (double)M_PI / 180.0;
	a.x = pow(ray->dir.x, 2) + pow(ray->dir.y, 2) -
		pow(ray->dir.z, 2) * pow(tan(r), 2);
	a.y = (2 * temp.x * ray->dir.x) + ( 2 * temp.y * ray->dir.y) -
		(2 * temp.z * ray->dir.z) * pow(tan(r), 2);
	a.z = pow(temp.x, 2) + pow(temp.y, 2) -
		pow(temp.z, 2) * pow(tan(r), 2);*/
